...
export type ITSEnumLike<K extends string = string, V extends string | number = string | number> =
	ITSReadonlyRecord<K, V>
	| (V extends number ? ITSNumberEnumLikeReverse<K, Exclude<V, string>> : ITSEnumLikeReverseExcludeNumber);
 ...
 */
export type ITSStringEnumLike<K extends string = string, V extends string = string> =
	(ITSReadonlyRecord<K, V> | ITSStringEnumLikeReverse<K, V>)
	& ITSEnumLikeReverseExcludeNumber;
 ...
 */
export type ITSNumberEnumLike<K extends string = string, V extends number = number> =
	ITSReadonlyRecord<K, V>
	| ITSNumberEnumLikeReverse<K, V>;
